                    ...
                    if ((self.size != 0 and fsize == 0) or
                        ...
                        with open(self.name, 'wb') as file:  # 显示进度条
                            for data in self.response.iter_content(chunk_size=GLOBAL_DOWN_MAX):
                                ...
                                now_t = time.time()
                                if self.size == 0:
                                    print('\r' + '[文件下载实时速度：%.2fKB/秒)' % (float(size / ((now_t - start) * 1024))), end=' ')
                                else:
...
def save_all_objects2():
    #sc = SharedCounter()
    #sc.save_all_objects()
    try:
        global GLOBAL_DOWN_LIST
        f = open(ROOT_OBJECT2, 'wb')
        pickle.dump(GLOBAL_DOWN_LIST, f)
    except Exception as e:
        print(e)
        write_except_file(e)
...
def get_all_threat():
    t = threading.Thread(target=download_file, name='LoopThread1')
    
    s = threading.Thread(target=download_file2, name='LoopThread2')
    s.start()
    t.start()
